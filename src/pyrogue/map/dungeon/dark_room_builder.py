...
class DarkRoomBuilder:
    ...
    def apply_darkness_to_rooms(
        self,
        rooms: list[Room],
        darkness_probability: float = 0.3
    ) -> list[DarkRoom]:
        """
        既存の部屋を暗い部屋に変換。
    ...
    def _convert_to_dark_room(self, room: Room) -> DarkRoom:
        ...
        # 暗い部屋を作成
        dark_room = DarkRoom(
            room.x, room.y, room.width, room.height, darkness_level
        )
    ...
    def get_light_influence_at(
        self,
        x: int, y: int,
        light_sources: list[tuple[int, int]] = None
    ) -> float:
        """
        指定位置での光の影響度を取得。
    ...
    def get_statistics(self) -> dict:
        ...
        return {
            ...
            "average_darkness_level": sum(room.darkness_level for room in self.dark_rooms) / len(self.dark_rooms) if self.dark_rooms else 0.0,
        }
