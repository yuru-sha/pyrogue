...
class InputHandler:
    ...
    def handle_key(self, event: tcod.event.KeyDown) -> GameStates | None:
        ...
        if command is None:
            command = chr(key) if isinstance(key, int) and 32 <= key <= 126 else text
    ...
    def _start_item_selection(self, action: str) -> GameStates | None:
        ...
        if not items:
            message = {"zap": "You have no wands to zap.", "read": "You have no scrolls to read."}.get(
                action, "You have nothing to throw."
            )
    ...
    def begin_direction_selection(self, action: str, item_id: int) -> None:
        """Wait for a direction before executing a selected item action."""
    ...
    def _handle_direction_selection_key(self, event: tcod.event.KeyDown) -> GameStates | None:
        ...
        if action is None or item_id is None:
            return None
        result = self.game_screen.execute(action, [item_id, _DIRECTION_NAMES[direction]])
