...
def test_inventory_display_bug():
    ...
    print(f"  攻撃指輪装備後: 左手={player.inventory.equipped['ring_left'].name if player.inventory.equipped['ring_left'] else None}")
    
    player.equip_item(defense_ring)
    print(f"  防御指輪装備後: 右手={player.inventory.equipped['ring_right'].name if player.inventory.equipped['ring_right'] else None}")
