...
def test_cursed_equipment():
    ...
    # GameContextを作成
    message_log = []
    context = GameContext(player=player, inventory=player.inventory, dungeon_manager=None, message_log=message_log)
    ...
    print(f"  インベントリ: {[f'{item.name}(cursed={item.cursed})' for item in player.inventory.items]}")
    print(
        f"  装備: {[(slot, f'{item.name}(cursed={item.cursed})' if item else None) for slot, item in player.inventory.equipped.items()]}"
    )
