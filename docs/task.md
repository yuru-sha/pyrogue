...
- [✅] **🎯戦術・UI拡張コマンド実装**（**CLI/GUI統合完了**）
    - [⚠️] **自動探索コマンド（Oキー）**【CLI/GUI両方で使用可能・実装は未完了】
        - [⚠️] 未探索エリアへの自動移動・探索（CLIでは未実装メッセージ表示）
        - [⚠️] 安全ルート検索とアイテム回収（CLIでは未実装メッセージ表示）
        - [⚠️] 敵発見時の自動停止機能（CLIでは未実装メッセージ表示）
        - [✅] 既存o（ドア開放）との競合解決
    ...
    - [✅] **キャラクター詳細表示（@キー）**【CLI/GUI両方で使用可能】
        ...
        - [✅] プレイヤー情報管理の向上
- [⚠️] **🔧システム機能修正・追加**（**部分的完了**）
    - [⚠️] **オートセーブ機能の実装確認**【GUIのみ・CLIでは未実装】
        - [⚠️] 環境変数連携の修正（GUIでは動作・CLIでは未実装）
        - [⚠️] 自動保存タイミングの設定（GUIでは動作・CLIでは未実装）
        - [⚠️] セーブファイル生成確認（GUIでは動作・CLIでは未実装）
    - [✅] **キー配置整理**（完了）
