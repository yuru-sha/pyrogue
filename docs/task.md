...
- [ ] **🚨重大バグ修正: セーブ・ロード機能**
    - [ ] SaveLoadManagerのメソッド名不一致修正
        - [ ] `save_game()` → `save_game_state()` への修正
        - [ ] `load_game()` → `load_game_state()` への修正  
        - [ ] `delete_save_file()` → `delete_save_data()` への修正
    - [ ] セーブファイル作成されない問題の解決
    - [ ] Ctrl+S、Ctrl+Lキーが動作しない問題の解決
- [ ] **🚨バグ修正: アイテム識別システム迂回**
    - [ ] MovementManager._check_item_pickup()の修正
    - [ ] `item.name` → `item.get_display_name(identification)` への変更
    - [ ] 未識別アイテムの移動時通知が正しく表示されるよう修正
- [x] **アイテム識別システム**
...
### 優先度：高
- [ ] **⭐基本ローグライクコマンド実装**
    - [ ] **ヘルプ表示コマンド（?キー）- メインゲーム画面**
        - [ ] 全コマンド一覧の表示
        - [ ] JIS配列キーボード対応（現在はインベントリのみ実装済み）
        - [ ] 新規プレイヤー向け学習支援
    - [ ] **調査・検査コマンド（xキー）**
        - [ ] カーソルでマップ上の任意地点を調査
        - [ ] モンスター・アイテム・地形の詳細情報表示
        - [ ] 戦術的判断支援機能
    - [ ] **シンボル説明コマンド（/キー）**
        - [ ] マップ上の文字・記号の意味説明
        - [ ] モンスター種類の識別支援
