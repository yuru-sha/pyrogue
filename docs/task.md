...
- [x] **トラップシステム**
    ...
    - [x] ダンジョン生成時の自動トラップ配置
- [x] **魔法的アイテムシステム**（※歴史的正確性要修正）
    - [x] 巻物による魔法効果（識別・テレポート・魔法の地図・光・呪い解除・武器強化・防具強化）
    - [x] ポーション効果（治癒・毒・能力強化等）
    - [x] 指輪効果（能力値修正・特殊効果）
    - [⚠️] **現在実装の問題点（要修正）**：
        - [⚠️] MP（マジックポイント）管理システム ← オリジナルRogueにはない
        - [⚠️] 呪文詠唱システム（Zキーで魔法メニュー） ← 正しくは「z = zap a wand」
        - [⚠️] 魔法書（Spellbook）による習得魔法管理 ← オリジナルRogueにはない
        - [⚠️] プレイヤーの直接魔法使用 ← オリジナルRogueでは不可能
- [🗑️] **NPCシステム**（削除予定 - オリジナルRogueに存在しない）
    - [🗑️] Actor基底クラス（Player、Monster、NPCの共通基盤）
    - [🗑️] NPCクラス（友好的・中立的・敵対的な態度管理）
    - [🗑️] NPC種類（商人、警備員、村人、僧侶、魔術師）
    - [🗑️] **NPCダンジョン配置システム**
        - [🗑️] NPCSpawnerクラス（階層に応じたNPC配置管理）
        - [🗑️] 特別な部屋でのNPC配置（商人→宝物庫、僧侶→聖域等）
        - [🗑️] フロアレベル別NPC数調整（深い階層ほど少ない）
        - [🗑️] ダンジョン生成時の自動NPC配置
        - [🗑️] 包括的テストスイート（13個のテストケース）
    - [🗑️] **プレイヤーとNPCの相互作用システム**
        - [🗑️] 対話コマンド（Tキー）による会話開始
        - [🗑️] プレイヤー周囲8方向のNPC検索
        - [🗑️] ゲームエンジンとの統合（DIALOGUE状態管理）
        - [🗑️] NPCの視覚的表示（ゲームマップ上）
    - [🗑️] 会話システム（DialogueManager）
        - [🗑️] 分岐対話システム
        - [🗑️] デフォルト会話データ（商人、警備員、僧侶、魔術師、村人）
        - [🗑️] 会話状態管理
        - [🗑️] NPC種類別対話ID管理
    - [🗑️] 取引システム（TradingManager）
        ...
        - [🗑️] 取引実行とエラーハンドリング
    - [🗑️] UI統合
        - [🗑️] 会話画面（DialogueScreen）
        - [🗑️] 取引画面（TradingScreen）
        - [🗑️] キーボード操作対応（T、矢印キー、Enter、Esc）
        - [🗑️] 状態管理とscreen遷移
- [x] **スコアランキングシステム**
...
- [🚨] **CLI/GUI統合システム**（重大な設計問題）
    - [x] CommonCommandHandlerの基本フレームワーク
    - [❌] CLI/GUI間の実際の動作統一（設計意図と実装が乖離）
    - [❌] セーブ・ロード処理の完全統一（現在は別実装）
    - [❌] 統合テストの実効性（現在は無意味化）

- [⚠️] **セーブ/ロード機能**（緊急修正が必要）
    - [x] 基本的なセーブ・ロード機能（Ctrl+S/Ctrl+L）
    - [❌] フロアデータの完全復元（現在はマップが再生成される）
    - [❌] 装備状態の正確な復元
    - [❌] プレイヤー位置の正確な復元
    - [❌] エンティティ配置の復元
...
### 最優先（ゲーム体験に直接影響）
- [ ] **🚨アーキテクチャの根本的問題: CLI/GUI統合化の失敗**
    - [ ] **問題の核心**: 同じコマンドハンドラーを使用しているはずなのに、CLIとGUIで動作が異なる
    - [ ] **具体的な問題**:
        - [ ] セーブ・ロード処理がCLIとGUIで完全に別実装
        - [ ] CLIテストが実際のGUI動作を反映していない
        - [ ] 統合テストが無意味化している
    - [ ] **必要な修正**:
        - [ ] CommonCommandHandlerの完全統合（現在は部分的統合のみ）
        - [ ] セーブ・ロード処理の完全統一（現在は全く別実装）
        - [ ] CLIとGUIの動作の完全一致の実現
        - [ ] 統合テストの実効性確保

- [ ] **セーブ/ロード機能の完全化**
    - [ ] フロアデータの完全復元システム（現在はマップが再生成される）
    - [ ] 装備状態の正確な復元（装備表示の不具合修正）
    - [ ] プレイヤー位置の正確な復元（壁にめり込み防止）
    - [ ] エンティティ配置の復元（トラップ、モンスター、アイテム）

- [ ] **装備システムの動作不具合修正**
    - [ ] 装備アイテムドロップ時の装備解除処理修正
    - [ ] 装備表示の正確性向上
    - [ ] 装備状態の一貫性確保
...
- [ ] **🚨歴史的正確性修正: 魔法システム**
    ...
    - [ ] オリジナルRogue準拠のワンドシステム実装
        ...
        - [ ] ワンドアイテムの実装（Fire Ball、Magic Missile、Teleport等）
        - [ ] 方向指定システムとワンド効果処理
    - [ ] 魔法的効果をアイテム経由のみに限定
...
**⚠️ 部分的実装・問題のある機能**：
- ⚠️ **セーブ/ロード機能**（基本動作するが重大な問題有り）
    - ❌ フロアデータの完全復元（マップ再生成される）
    - ❌ 装備状態の正確な復元（表示不具合）
    - ❌ プレイヤー位置の正確な復元（壁にめり込み等）
    - ❌ エンティティ配置の復元（トラップ、モンスター、アイテム）
- ⚠️ **基本ローグライクコマンド**（一部実装済み）
