...
- [x] **🎯戦術・UI拡張コマンド実装**（完了）
    ...
    - [x] **アイテム識別状況表示（\キー）**
        ...
        - [x] 識別・未識別状況の確認
        - [x] ゲーム進行状況の把握支援
    - [x] **最後のメッセージ表示（Ctrl+Mキー）**
        - [x] 見逃したメッセージの再表示
        - [x] メッセージ履歴のスクロール機能
        - [x] 重要情報の見落とし防止
    - [x] **投げるコマンド（tキー）**
        - [x] アイテム投擲による遠距離攻撃
        - [x] 投擲可能アイテムの判定（ダガー、槍、ポーション等）
        - [x] 軌道計算とダメージ処理
        - [x] NPCキー（tキー）の再配置が必要 → **NPCシステム削除により解決済み**
    - [x] **足元・周囲調査コマンド（lキー：GUIモード）**
        - [x] カーソルで任意地点の調査
        - [x] アイテム・モンスター・地形の詳細表示
        - [x] プレイヤー足元のアイテム確認機能
    - [x] **直接装備コマンド（wキー）**
        - [x] メイン画面から直接装備操作
        - [x] インベントリ経由不要の効率化
...
- [x] **🔧システム機能修正・追加**（完了）
    - [x] **オートセーブ機能の実装確認**（完了）
        ...
        - [x] セーブファイル生成確認
    - [x] **キー配置整理**（完了）
        - [x] tキーコンフリクト解決（投げる vs NPC対話）→ NPCシステム削除により解決済み
        - [x] NPCキーを別キー（例：T）に移動 → NPCシステム削除により不要
        - [x] ヘルプ画面の更新 → 戦術・UI拡張コマンド実装時に完了
- [x] **コード整理・品質向上**
    - [x] バックアップファイルの整理・削除
    - [x] テスト失敗の修正（幻覚システム関連）
    - [x] コードベースの整理・統一化
