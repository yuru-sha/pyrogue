...
`DungeonGenerator` はseed付きRNGを受け取り、全階で3×3区画の部屋を生成します。原作同様に
区画を欠落させ、接続グラフで部屋を通路につなぎ、一部の部屋を迷路または暗室にします。
暗室内の視界は隣接範囲に制限し、light効果で照らした部屋は全体を可視化します。生成後に
歩行可能領域から階段への到達性を検査し、壊れた階はゲーム開始前にエラーにします。
...
`0.3.8` と一致しないファイルは自動移行せず、互換性エラーとして拒否します。階層内の
未発見の隠し扉・通路、罠の発見状態、アイテム識別・呼称、一時効果、モンスターの行動状態、
食料生成間隔、失神・bear restraint・睡眠残りターンも保存します。`current_floor` は現在位置、
