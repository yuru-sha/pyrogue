...
def test_door_placement():
    ...
    for room in generator.rooms:
        ...
        for door_x, door_y in room.doors:
            # 扉が壁の中央にあるか確認
            if door_y == room.y:  # 北壁
                assert door_x == room.x + room.width // 2, "Door not in center of north wall"
                wall_doors["north"] += 1
            elif door_y == room.y + room.height - 1:  # 南壁
                assert door_x == room.x + room.width // 2, "Door not in center of south wall"
                wall_doors["south"] += 1
            elif door_x == room.x:  # 西壁
                assert door_y == room.y + room.height // 2, "Door not in center of west wall"
                wall_doors["west"] += 1
            elif door_x == room.x + room.width - 1:  # 東壁
                assert door_y == room.y + room.height // 2, "Door not in center of east wall"
                wall_doors["east"] += 1
...
def test_stairs_placement():
    ...
    for room in generator.rooms:
        if (start_pos[0] > room.x and start_pos[0] < room.x + room.width - 1 and
            start_pos[1] > room.y and start_pos[1] < room.y + room.height - 1):
            start_room = room
        
        if (end_pos[0] > room.x and end_pos[0] < room.x + room.width - 1 and
            end_pos[1] > room.y and end_pos[1] < room.y + room.height - 1):
            end_room = room
...
def test_door_types():
    ...
    if total_dead_end_doors > 0:  # 分母が0でないことを確認
        ...
        assert 0.15 <= dead_end_normal_ratio <= 0.35, "Dead end normal door ratio out of expected range"
                    ...
                    if (next_x, next_y) in generator.corridors:
                        ...
                        while (next_x, next_y) in generator.corridors:
                            ...
                            for ndx, ndy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                                ...
                                if (new_x, new_y) in generator.corridors and (new_x, new_y) not in special_corridor:
                ...
                for x, y in special_corridor:
                    ...
                    for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                        if (x + dx, y + dy) in other_corridors:
                            assert False, "Special room corridor intersects with other corridor" 
...
def test_door_on_walls_only():
    ...
    for room in generator.rooms:
        ...
        for door_x, door_y in room.doors:
            ...
            # 扉が壁上にあることを確認
            is_on_wall = (
                (door_y == room.y or door_y == room.y + room.height - 1) or  # 北壁または南壁
                (door_x == room.x or door_x == room.x + room.width - 1)      # 西壁または東壁
            )
...
def test_dead_end_corridors():
    ...
    for x, y in generator.corridors:
        ...
        for dx, dy in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
            ...
            if (next_x, next_y) in generator.corridors or \
               isinstance(generator.tiles[next_y, next_x], (Door, SecretDoor)):
                connections += 1
    ...
    for dead_end_x, dead_end_y in dead_ends:
        for room in generator.rooms:
            assert not (room.x < dead_end_x < room.x + room.width - 1 and 
                       room.y < dead_end_y < room.y + room.height - 1), \
...
def test_special_room_generation():
    ...
    for floor in special_floor_levels:
        ...
        # 特別な部屋が1つ存在することを確認
        assert len(special_rooms) == 1, f"Floor {floor} should have exactly one special room"
        
        special_room = special_rooms[0]
        
        # 部屋のタイプが正しいことを確認
        assert special_room.room_type in [
            "treasure", "armory", "food", "monster", "library", "laboratory"
...
def test_map_boundary():
    ...
    for x in range(generator.width):
        ...
        assert isinstance(generator.tiles[generator.height-1, x], Wall), "Bottom boundary is not wall"
    
    # 左右の境界をチェック
    for y in range(generator.height):
        assert isinstance(generator.tiles[y, 0], Wall), "Left boundary is not wall"
        assert isinstance(generator.tiles[y, generator.width-1], Wall), "Right boundary is not wall"
...
def test_stairs_in_room_interior():
    ...
    up_stairs_in_interior = any(
        is_in_room_interior(start_pos, room)
        for room in generator.rooms
    )
    ...
    down_stairs_in_interior = any(
        is_in_room_interior(end_pos, room)
        for room in generator.rooms
    )
    ...
    for y in range(50):
        for x in range(80):
            if isinstance(tiles[y, x], Floor) and (x, y) in dungeon.corridors:
                ...
                assert x > 2 and x < 77, f"Corridor at ({x}, {y}) is too close to horizontal boundary"
                assert y > 2 and y < 47, f"Corridor at ({x}, {y}) is too close to vertical boundary"
    ...
    for y in range(1, 49):
        for x in range(1, 79):
            if isinstance(tiles[y, x], Floor) and (x, y) in dungeon.corridors:
                ...
                if wall_count >= 3:
                    # 境界からの距離が2マス以上あることを確認
                    assert x > 2 and x < 77, f"Dead end at ({x}, {y}) is too close to horizontal boundary"
                    assert y > 2 and y < 47, f"Dead end at ({x}, {y}) is too close to vertical boundary"
