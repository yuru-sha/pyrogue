...
from pyrogue.core.command_handler import CommonCommandHandler, CommandResult, CommandContext
...
class MockCommandContext(CommandContext):
    ...
    def __init__(self):
        ...
        self.game_logic.inventory.equipped = {
            "weapon": None,
            "armor": None,
            "ring_left": None,
            "ring_right": None
...
class TestCommonCommandHandler:
    ...
    def test_auto_explore_with_enemies(self):
        ...
        # 近くに敵を配置
        floor_data = self.context.game_logic.get_current_floor_data()
        enemy = Monster(x=6, y=6, name="Goblin", char="g", hp=10, max_hp=10, 
                       attack=5, defense=2, level=1, exp_value=50, 
                       view_range=3, color=(255, 0, 0))
    ...
    def test_save_load_data_integrity(self):
        ...
        with patch('pyrogue.core.save_manager.SaveManager') as mock_save_manager:
            ...
            # フロアにモンスターとアイテムを配置
            floor_data = self.context.game_logic.get_current_floor_data()
            test_monster = Monster(x=10, y=10, name="Orc", char="O", hp=30, max_hp=30,
                                 attack=15, defense=8, level=3, exp_value=75, 
                                 view_range=4, color=(0, 255, 0))
